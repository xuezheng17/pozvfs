...
HandlePageStatCeremony.prototype._retrieveResults = function(page, condition, from, to) {
  var _self = this;
  var args = '';
  new RequestUtils()._mysql('iestatsceremony', args, function(result, params) { _self._results = result;
...
HandlePageStatCeremony.prototype._updateElements = function() {
  ...
  for (var i = 0, il = this._results.length; i < il; i++) {
    ...
    a.onclick = function() { var w = window.open('?p=pageasst&t=pagestatceremonydetail&opts=' + JSON.stringify({ name: this._name }), 'Ceremony', 'width=1024,height=768,scrollbars=1,location=0', true);
                             w.focus();
                           };
    td.appendChild(a);
  }
