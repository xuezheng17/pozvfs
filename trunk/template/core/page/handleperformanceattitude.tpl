function HandlePerformanceAttitude(gui, operator, now, options) {
  this._gui = gui;
...
HandlePerformanceAttitude.prototype._createElements = function() {
...
HandlePerformanceAttitude.prototype._loadData = function() {
...
HandlePerformanceAttitude.prototype._verifyData = function() {
  if (this._operations && this._dateZones) {
    this._updateElements();
  }
};

HandlePerformanceAttitude.prototype._retrieveOperations = function(page, condition, from, to) {
  var _self = this;
  var args = ((condition) ? '&c=' + condition : '') + '&p=' + page + '&s=99999' + '&from=' + ((from) ? from : '' ) + '&to=' + ((to) ? to : '');
  new RequestUtils()._mysql('performanceAttitude', args, function(result, params) { _self._operations = result.data;
...
HandlePerformanceAttitude.prototype._operationSearch = function(gui, callbackFunc) {
  var _self = this;
 ...
 * 修改日期1Change
 */
HandlePerformanceAttitude.prototype._changeDate = function(label, currentDate, onChangeFunc, showTime) {
  var div, _self = this;
 ...
 * 修改日期2Select
 */
HandlePerformanceAttitude.prototype._selectDate = function(sd, label, onChangeFunc, showTime) {
  onChangeFunc(sd);
 ...
 * 修改日期3Show
 */
HandlePerformanceAttitude.prototype._showDate = function(sd, label, showTime) {
  if (sd) { 
