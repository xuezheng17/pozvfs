...
<script type="text/javascript">
  <!--
    ...
    function init() {
      ...
      var gui = AbstractStatBasic.gui('{{$template|escape:javascript}}');
      var options = { template: '{{$template|escape:'javascript'}}', option: '{{$options|escape:javascript}}' };
      new HandleStatBasic(gui, MiscUtils.decode('{{$operator|escape:javascript}}'), MiscUtils.decode('{{$now|escape:javascript}}'), options);
    }
  //-->
</script>
  ...
  <tr>
    <td class="{{$template|escape:'html'}}_td4" colSpan="3">
      <table class="{{$template|escape:'html'}}_table3" cellPaddiing="0" cellSpacing="0" border=0>
        ...
        <tr>
          <td style="height:570px;vertical-align:top" id="{{$template|escape:'html'}}_graphy_div" colSpan="6"></td>
      </table>
    </td>
