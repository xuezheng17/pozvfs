...
<script type="text/javascript">
  <!--
    ...
    function init() {
      ...
      var gui = AbstractCategorys.gui('{{$template|escape:javascript}}');
      new HandleCategorys(gui, MiscUtils.decode('{{$operator|escape:javascript}}'), MiscUtils.decode('{{$now|escape:javascript}}'), MiscUtils.decode('{{$options|escape:javascript}}'));
    }
  //-->
</script>
      ...
      <table class="{{$template|escape:'html'}}_table3" cellPaddiing="0" cellSpacing="0"border=0>
        <tr>
          <td class="{{$template|escape:'html'}}_td7">
            <a id="{{$template|escape:'html'}}_add_link">Create new category</a>
          </td>
        </tr>
      </table>
