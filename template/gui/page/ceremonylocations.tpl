...
<script type="text/javascript">
  <!--
    ...
    function init() {
      ...
      var gui = AbstractCeremonyLocations.gui('{{$template|escape:javascript}}');
      new HandleCeremonyLocations(gui, MiscUtils.decode('{{$operator|escape:javascript}}'), MiscUtils.decode('{{$now|escape:javascript}}'), MiscUtils.decode('{{$options|escape:javascript}}'));
    }
  //-->
</script>
      ...
      <table class="{{$template|escape:'html'}}_table3" cellPaddiing="0" cellSpacing="0"border=0>
        <tr>
          <td class="{{$template|escape:'html'}}_td7">
            <a id="{{$template|escape:'html'}}_add_link">Create new  Ceremony Location</a>
          </td>
        </tr>
      </table>
