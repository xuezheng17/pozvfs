function HandlePerformanceSales(gui, operator, now, options) {
  this._gui = gui;
...
HandlePerformanceSales.prototype._createElements = function() {
...
HandlePerformanceSales.prototype._loadData = function() {
...
HandlePerformanceSales.prototype._verifyData = function() {
  if (this._operations && this._dateZones) {
    this._updateElements();
  }
};

HandlePerformanceSales.prototype._retrieveOperations = function(page, condition, from, to) {
  var _self = this;
  var args = ((condition) ? '&c=' + condition : '') + '&p=' + page + '&s=99999' + '&from=' + ((from) ? from : '' ) + '&to=' + ((to) ? to : '');
  new RequestUtils()._mysql('performanceSales', args, function(result, params) { _self._operations = result.data;
                                                                                 _self._verifyData.call(_self);
                                                                               }, null);
};

HandlePerformanceSales.prototype._retrieveDateZones = function() {
  var _self = this;
...
HandlePerformanceSales.prototype._updateElements = function() {
...
HandlePerformanceSales.prototype._operationSearch = function(gui, callbackFunc) {
  var _self = this;
 ...
 * 修改日期1Change
 */
HandlePerformanceSales.prototype._changeDate = function(label, currentDate, onChangeFunc, showTime) {
  var div, _self = this;
 ...
 * 修改日期2Select
 */
HandlePerformanceSales.prototype._selectDate = function(sd, label, onChangeFunc, showTime) {
  onChangeFunc(sd);
 ...
 * 修改日期3Show
 */
HandlePerformanceSales.prototype._showDate = function(sd, label, showTime) {
  if (sd) { 
