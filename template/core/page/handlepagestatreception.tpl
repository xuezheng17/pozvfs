...
HandlePageStatReception.prototype._retrieveResults = function(page, condition, from, to) {
  var _self = this;
  var args = ((condition) ?  '&c=' + condition : '') + '&p=' + page + ((from) ?  '&from=' + from : '') + ((to) ?  '&to=' + to : '');
  new RequestUtils()._mysql('statsreception', args, function(result, params) { _self._results = result;
                                                                                 _self._verifyData.call(_self);
                                                                               }, null);
};

HandlePageStatReception.prototype._retrieveDateZones = function() {
  var _self = this;
...
HandlePageStatReception.prototype._updateElements = function() {
  ...
  for (var i = 0, il = this._results.length; i < il; i++) {
    ...
    a.onclick = function() { var pos, func1;
                             ...
                             new ModuleDialogRecpCus(document, tmp._gui.panel, 300, 30, _self._operator, _self._now, { popupBox: tmp, pos: DOMUtils.findPos(this),name:this. _name });
                             return false;
                           };
    td.appendChild(a);
  }
...
HandlePageStatReception.prototype._customerSearch = function(gui, callbackFunc) {
  var _self = this;
...
HandlePageStatReception.prototype._changeDate = function(label, currentDate, onChangeFunc, showTime) {
  var div, _self = this;
...
HandlePageStatReception.prototype._selectDate = function(sd, label, onChangeFunc, showTime) {
  onChangeFunc(sd);
  this._showDate(sd, label, showTime);
  this._popupBox._close();
};

HandlePageStatReception.prototype._showDate = function(sd, label, showTime) {
  if (sd) { 
